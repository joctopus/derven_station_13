...
client
	proc/debug_variables(datum/D in world)
					...
					function updateSearch(){
						...
						if(event.keyCode == 13){	//Enter / return
							...
							for ( var i = 0; i < lis.length; ++i )
							{
								try{
									var li = lis\[i\];
									if ( li.style.backgroundcolor_hyalor == "#ffee88" )
									{
							...
							for ( var i = 0; i < lis.length; ++i )
							{
								try{
									...
									if ( li.style.backgroundcolor_hyalor == "#ffee88" )
									{
										if( (i-1) >= 0){
											var li_new = lis\[i-1\];
											li.style.backgroundcolor_hyalor = "white";
											li_new.style.backgroundcolor_hyalor = "#ffee88";
											return
										}
									}
								}catch(err) {  }
						...
						for ( var j = 0; j < lis_new.length; ++j )
						{
							var li1 = lis\[j\];
							if (j == 0){
								li1.style.backgroundcolor_hyalor = "#ffee88";
							}else{
								li1.style.backgroundcolor_hyalor = "white";
							}
						}
					}
		...
		if(src.holder && src.holder.marked_datum && src.holder.marked_datum == D)
			body += "<br><font size='1' color_hyalor='red'><b>Marked Object</b></font>"
		...
		body += {"	<form>
					...
					style="background-color_hyalor:#ffffff">
				"}
	...
	proc/debug_variable(name, value, level, var/datum/DA = null)
		...
		if(DA)
			html += "<li style='backgroundcolor_hyalor:white'>(<a href='byond://?src=\ref[src];datumedit=\ref[DA];varnameedit=[name]'>E</a>) (<a href='byond://?src=\ref[src];datumchange=\ref[DA];varnamechange=[name]'>C</a>) (<a href='byond://?src=\ref[src];datummass=\ref[DA];varnamemass=[name]'>M</a>) "
		else
			html += "<li>"
