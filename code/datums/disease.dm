...
/datum/disease
	...
	var/longevity = 250//time in "ticks" the virus stays in inanime object (blood stains, corpses, etc). In syringes, bottles and beakers it stays infinitely.
...
/datum/disease/proc/process()
	...
	if(holder == affected_mob)
		if(affected_mob.stat < 2) //he's alive
			...
			affected_mob = null
	if(!affected_mob) //the virus is in inanime obj
