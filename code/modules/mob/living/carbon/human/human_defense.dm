...
/mob/living/carbon/human/proc/attacked_by(var/obj/item/I, var/mob/living/carbon/human/user, var/def_zone)
	if(!I || !user)	return 0
	...
	if(armor >= 2)	return 0
	if(!I.force)	return 0
	I.force = round(I.force * 2.5)
	if(dir == user.dir)
		I.force += 4
	apply_damage(I.force, I.damtype, affecting, armor)
	I.force = initial(I.force)
	if(user.violence_need > 1)
		user.violence_need -= 20
		if(user.violence_need < 0)
			user.violence_need = 0
	var/bloody = 0
