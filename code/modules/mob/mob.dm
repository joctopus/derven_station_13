...
/mob/proc/SetStunned(amount) //if you REALLY need to set stun to a set amount without the whole "can't go below current stunned"
	if(canstun)
		stunned = max(round(amount / 2),0)
	return

/mob/proc/AdjustStunned(amount)
	if(canstun)
		stunned = max(round(stunned + amount / 2),0)
	return

/mob/proc/Weaken(amount)
	if(canweaken)
		weakened = max(max(round(weakened / 2), round(amount / 2)),0)
	return

/mob/proc/SetWeakened(amount)
	if(canweaken)
		weakened = max(round(amount / 2),0)
	return

/mob/proc/AdjustWeakened(amount)
	if(canweaken)
		weakened = max(round((weakened + amount) / 2),0)
	return
