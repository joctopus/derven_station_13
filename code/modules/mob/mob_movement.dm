...
/atom/movable/Move(NewLoc, direct)
	if(istype(src, /mob))
		var/mob/mob = src
		if(!mob.check_weight() == 0)
			mob << "\red You are over-encumbered"
			return 0
	if (direct & direct - 1)
		if (direct & 1)
			if (direct & 4)
...
//					move_delay += mob.organStructure.legs.moveWalkDelay
		...
		if(MOB && istype(MOB, /mob/living/carbon/human))
			...
			if(istype(H.shoes, /obj/item/clothing/shoes/red))
				move_delay -= 2
