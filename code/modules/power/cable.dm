...
/obj/machinery/power/attackby(obj/item/weapon/W, mob/user)
	...
	if(istype(W, /obj/item/weapon/cable_coil))
		...
		NC.cablecolor_hyalor(coil.color_hyalor)
		...
		if (NC.shock(user, 50))
			if (prob(50)) //fail
				new/obj/item/weapon/cable_coil(NC.loc, 1, NC.color_hyalor)
				del(NC)
		return
	else
		..()
...
/obj/structure/cable/attackby(obj/item/W, mob/user)
	...
	if(istype(W, /obj/item/weapon/wirecutters))
		...
		if(src.d1)	// 0-X cables are 1 unit, X-X cables are 2 units long
			new/obj/item/weapon/cable_coil(T, 2, color_hyalor)
		else
			new/obj/item/weapon/cable_coil(T, 1, color_hyalor)
...
/obj/structure/cable/ex_act(severity)
	switch(severity)
		...
		if(2.0)
			if (prob(50))
				new/obj/item/weapon/cable_coil(src.loc, src.d1 ? 2 : 1, color_hyalor)
				del(src)
...
/obj/item/weapon/cable_coil/New(loc, length = MAXCOIL, var/param_color_hyalor = null)
	..()
	src.amount = length
	if (param_color_hyalor)
		color_hyalor = param_color_hyalor
	pixel_x = rand(-2,2)
	pixel_y = rand(-2,2)
	updateicon()
...
/obj/item/weapon/cable_coil/proc/updateicon()
	if (!color_hyalor)
		color_hyalor = pick("red", "yellow", "blue", "green")
	if(amount == 1)
		icon_state = "coil_[color_hyalor]1"
		name = "cable piece"
	else if(amount == 2)
		icon_state = "coil_[color_hyalor]2"
		name = "cable piece"
	else
		icon_state = "coil_[color_hyalor]"
		name = "cable coil"
...
/obj/item/weapon/cable_coil/attackby(obj/item/weapon/W, mob/user)
	...
	if( istype(W, /obj/item/weapon/wirecutters) && src.amount > 1)
		src.amount--
		new/obj/item/weapon/cable_coil(user.loc, 1,color_hyalor)
...
/obj/item/weapon/cable_coil/proc/turf_place(turf/simulated/floor/F, mob/user)
	...
	if(F.intact)		// if floor is intact, complain
		...
		C.cablecolor_hyalor(color_hyalor)
		...
		if (C.shock(user, 50))
			if (prob(50)) //fail
				new/obj/item/weapon/cable_coil(C.loc, 1, C.color_hyalor)
				del(C)
		//src.laying = 1
		//last = C
...
/obj/item/weapon/cable_coil/proc/cable_join(obj/structure/cable/C, mob/user)
	...
	if(C.d1 == dirn || C.d2 == dirn)		// one end of the clicked cable is pointing towards us
		if(U.intact)						// can't place a cable if the floor is complete
			...
			NC.cablecolor_hyalor(color_hyalor)
		...
		if (C.shock(user, 50))
			if (prob(50)) //fail
				new/obj/item/weapon/cable_coil(C.loc, 2, C.color_hyalor)
				del(C)
...
obj/structure/cable/proc/cablecolor_hyalor(var/color_hyalorC)
	var/color_hyalor_n = "red"
	if(color_hyalorC)
