...
datum
	chemical_reaction
		...
		//potassium + phosphorous + sulfur + plasma
		//potassium + nitrogen + oxygen + sulfur
		...
		match_bomb
			...
			id = "match_bomb"
			result = null
			required_reagents = list("potassium" = 1, "phosphorus" = 1, "sulfur" = 1, "plasma" = 1)
			result_amount = 2
			...
			on_reaction(var/datum/reagents/holder, var/created_volume)
				...
				for(var/turf/simulated/floor/target_tile in range(0,location))
					...
					var/datum/gas_mixture/match_bomb = new

					match_bomb.toxins = created_volume
					match_bomb.temperature = 200+T0C

					target_tile.assume_air(match_bomb)
					spawn (0) target_tile.hotspot_expose(500, 200)
				holder.del_reagent("match_bomb")
				return
		...
		powder
			...
			id = "powder"
			result = null
			required_reagents = list("potassium" = 1, "nitrogen" = 1, "oxygen" = 1, "sulfur" = 1)
			result_amount = 2
			...
			on_reaction(var/datum/reagents/holder, var/created_volume)
				...
				for(var/turf/simulated/floor/target_tile in range(0,location))
					...
					var/datum/gas_mixture/powder = new

					powder.toxins = created_volume
					powder.temperature = 100+T0C

					target_tile.assume_air(powder)
					spawn (0) target_tile.hotspot_expose(300, 100)

				var/datum/effect/effect/system/reagents_explosion/e = new()
				e.set_up(round(created_volume/2, created_volume/5), location, round(created_volume/2, created_volume/5), 0)
				e.start()

				holder.del_reagent("powder")
				return
