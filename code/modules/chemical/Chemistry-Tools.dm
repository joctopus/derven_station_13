...
/obj/item/weapon/reagent_containers/hypospray/attack(mob/M as mob, mob/user as mob)
	...
	if (reagents.total_volume)
		...
		M.attack_log += text("\[[time_stamp()]\] <font color_hyalor='orange'>Has been injected with [src.name] by [user.name] ([user.ckey])</font>")
		user.attack_log += text("\[[time_stamp()]\] <font color_hyalor='red'>Used the [src.name] to inject [M.name] ([M.ckey])</font>")

		log_attack("<font color_hyalor='red'>[user.name] ([user.ckey]) injected [M.name] ([M.ckey]) with [src.name] (INTENT: [uppertext(user.a_intent)])</font>")
		...
		if(istype(M, /mob/living/carbon))
			if(M == user)								//If you're eating it yourself.
				...
				if(!istype(M, /mob/living/carbon/metroid))		//If you're feeding it to someone else.
					...
					M.attack_log += text("\[[time_stamp()]\] <font color_hyalor='orange'>Has been fed [src.name] by [user.name] ([user.ckey]) Reagents: \ref[reagents]</font>")
					user.attack_log += text("\[[time_stamp()]\] <font color_hyalor='red'>Fed [M.name] by [M.name] ([M.ckey]) Reagents: \ref[reagents]</font>")

					log_attack("<font color_hyalor='red'>[user.name] ([user.ckey]) fed [M.name] ([M.ckey]) with [src.name] (INTENT: [uppertext(user.a_intent)])</font>")
	...
	on_reagent_change(var/mob/user)
		...
		*/
		...
		if(reagents.total_volume)
			...
			for(var/datum/reagent/re in reagents.reagent_list) // natural color_hyalor mixing bullshit/algorithm
			...
			overlay.icon += finalcolor_hyalor
...
/obj/item/weapon/reagent_containers/food/condiment	//Food items that aren't eaten normally and leave an empty container behind.
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		...
		if(M == user)
			...
			M.attack_log += text("\[[time_stamp()]\] <font color_hyalor='orange'>Has been fed [src.name] by [user.name] ([user.ckey]) Reagents: \ref[reagents]</font>")
			user.attack_log += text("\[[time_stamp()]\] <font color_hyalor='red'>Fed [src.name] by [M.name] ([M.ckey]) Reagents: \ref[reagents]</font>")
...
/obj/structure/reagent_dispensers/cleanertank
	...
	New()
		..()
		reagents.add_reagent("space_cleaner",1000)
...
/obj/structure/reagent_dispensers/powdertank
	name = "powdertank"
	desc = "A powdertank"
	icon = 'objects.dmi'
	icon_state = "powdertank"
	amount_per_transfer_from_this = 10
	New()
		..()
		reagents.add_reagent("nitrogen",250)
		reagents.add_reagent("oxygen",250)
		reagents.add_reagent("sulfur",250)

/obj/structure/reagent_dispensers/heliumtank
	name = "heliumtank"
	desc = "A heliumtank"
	icon = 'objects.dmi'
...
/obj/structure/reagent_dispensers/potassiumtank
	name = "potassiumtank"
	desc = "A potassiumtank"
	icon = 'objects.dmi'
	icon_state = "potassiumtank"
	amount_per_transfer_from_this = 10
	New()
		..()
		reagents.add_reagent("potassium",1000)

/obj/structure/reagent_dispensers/arsenictank
	name = "arsenictank"
	desc = "A arsenictank"
	icon = 'objects.dmi'
	icon_state = "arsenictank"
	amount_per_transfer_from_this = 10
	New()
		..()
		reagents.add_reagent("arsenic",1000)
		...
		if (reagents.reagent_list.len > 0)
			...
			switch(reagents.get_master_reagent_id())
				...
				if("tequilla")
					...
					desc = "Now all that's missing is the weird color_hyalored shades!"
				if("patron")
				...
				if("red_mead")
					...
					name = "Red Mead"
					desc = "A True Vikings Beverage, though its color_hyalor is strange."
				if("mead")
