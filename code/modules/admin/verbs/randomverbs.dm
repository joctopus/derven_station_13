	...
	if(M)
		...
		for(var/mob/K in world)	//we don't use message_admins here because the sender/receiver might get it too
			if(K && K.client && K.client.holder && K.key != usr.key && K.key != M.key)
				K << "<B><font color_hyalor='blue'>PM: [key_name(usr, K)]-&gt;[key_name(M, K)]:</B> \blue [t]</font>"
...
/proc/create_xeno(mob/dead/observer/G)
	...
	alert("There are no available ghosts to throw_hyuow into the xeno. Aborting command.")
	del(new_xeno)
	return
...
/client/proc/respawn_character()
	...
	*/
	...
	new_character.key = player_key//throw_hyuow them into the mob.
