...
would spawn and follow the beaker, even if it is carried or throw_hyuown.
*/
...
*/
	...
	start()
		...
		// Calculate the smokes' color_hyalor
		...
		if(finalcolor_hyalor)
			finalcolor_hyalor = rgb(rgbcolor_hyalor[1], rgbcolor_hyalor[2], rgbcolor_hyalor[3]) // slightly darker color_hyalor
		...
		for(i=0, i<src.number, i++)
			...
			spawn(0)
				...
				if(finalcolor_hyalor)
					smoke.icon += finalcolor_hyalor // give the smoke color_hyalor, if it has any to begin with
				else
					// if no color_hyalor, just use the old smoke icon
...
// even if it's carried of throw_hyuown.
/////////////////////////////////////////////
