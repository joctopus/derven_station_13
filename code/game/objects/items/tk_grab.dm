...
*/
	...
	afterattack(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, flag)//TODO: go over this
		...
		if(last_throw_hyuow+4 > world.time)	return
		if(!host)
			del(src)
			return
		...
		if((get_dist(target, user) <= 16) && (get_dist(focus, user) <= 10))
			apply_focus_overlay()
			focus.throw_hyuow_at(target, 10, 1)
			last_throw_hyuow = world.time
		return


	proc/focus_object(var/obj/target)
		if(!istype(target,/obj))	return//Cant throw_hyuow non objects atm might let it do mobs later
		if(target.anchored)	return//No throw_hyuowing anchored things
		focus = target
		update_icon()
		apply_focus_overlay()
		return
