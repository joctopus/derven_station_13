/obj/item/weapon/flamethrow_hyuower/full/New(var/loc)
	..()
...
/obj/item/weapon/flamethrow_hyuower
	name = "flamethrower"
	...
	update_icon()
		...
		if(lit)
			src.overlays += "+lit"
			item_state = "flamethrow_hyuower_1"
		else
			item_state = "flamethrow_hyuower_0"
		return
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if(istype(W,/obj/item/weapon/tank/plasma))
			if(src.ptank)
				user << "\red There appears to already be a plasma tank loaded in the flamethrow_hyuower!"
				return
			src.ptank = W
			W.loc = src
			if (user.client)
	...
	attack_self(mob/user as mob)
		...
		if (!src.ptank)
			...
			return
		var/dat = text("<TT><B>Flamethrow_hyuower (<A HREF='?src=\ref[src];light=1'>[lit ? "<font color_hyalor='red'>Lit</font>" : "Unlit"]</a>)</B><BR>\n Tank Pressure: [src.ptank.air_contents.return_pressure()]<BR>\nAmount to throw_hyuow: <A HREF='?src=\ref[src];amount=-100'>-</A> <A HREF='?src=\ref[src];amount=-10'>-</A> <A HREF='?src=\ref[src];amount=-1'>-</A> [src.throw_hyuow_amount] <A HREF='?src=\ref[src];amount=1'>+</A> <A HREF='?src=\ref[src];amount=10'>+</A> <A HREF='?src=\ref[src];amount=100'>+</A><BR>\n<A HREF='?src=\ref[src];remove=1'>Remove plasmatank</A> - <A HREF='?src=\ref[src];close=1'>Close</A></TT>")
		user << browse(dat, "window=flamethrow_hyuower;size=600x300")
		onclose(user, "flamethrow_hyuower")
		return
	...
	Topic(href,href_list[])
		if (href_list["close"])
			usr.machine = null
			usr << browse(null, "window=flamethrow_hyuower")
			return
		...
		if (href_list["light"])
			...
			if(lit)
				...
				damtype = "brute"
		if (href_list["amount"])
			src.throw_hyuow_amount = src.throw_hyuow_amount + text2num(href_list["amount"])
			src.throw_hyuow_amount = max(50,min(5000,src.throw_hyuow_amount))
		if (href_list["remove"])
			if(!src.ptank)	return
...
/obj/item/weapon/flamethrow_hyuower/proc/flame_turf(turflist)
...
/obj/item/weapon/flamethrow_hyuower/proc/ignite_turf(turf/target)
