...
/obj/item/weapon/dnainjector/attack(mob/M as mob, mob/user as mob)
	...
	if (!(istype(usr, /mob/living/carbon/human) || ticker) && ticker.mode.name != "monkey")
		...
		return
	M.attack_log += text("\[[time_stamp()]\] <font color_hyalor='orange'>Has been injected with [name] by [user.name] ([user.ckey])</font>")
	user.attack_log += text("\[[time_stamp()]\] <font color_hyalor='red'>Used the [name] to inject [M.name] ([M.ckey])</font>")

	log_attack("<font color_hyalor='red'>[user.name] ([user.ckey]) used the [name] to inject [M.name] ([M.ckey])</font>")
