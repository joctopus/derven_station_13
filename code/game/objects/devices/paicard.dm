	...
	attack_self(mob/user)
		...
		if(pai)
			...
			if(radio)
				...
				dat += "<b>Radio Uplink</b><br>"
				dat += "<font color_hyalor=red><i>Radio firmware not loaded. Please install a pAI personality to load firmware.</i></font><br>"
	...
	Topic(href, href_list)
		...
		if(href_list["setdna"])
			...
			if(!istype(M, /mob/living/carbon))
				usr << "<font color_hyalor=blue>You don't have any DNA, or your DNA is incompatible with this device.</font>"
			else
				var/datum/dna/dna = usr.dna
				pai.master = M.real_name
				pai.master_dna = dna.unique_enzymes
				pai << "<font color_hyalor = red><h3>You have been bound to a new master.</h3></font>"
		...
		if(href_list["wipe"])
			...
			if(confirm == "Yes")
				for(var/mob/M in src)
					M << "<font color_hyalor = #ff0000><h2>You feel yourself slipping away from reality.</h2></font>"
					M << "<font color_hyalor = #ff4d4d><h3>Byte by byte you lose your sense of self.</h3></font>"
					M << "<font color_hyalor = #ff8787><h4>Your mental faculties leave you.</h4></font>"
					M << "<font color_hyalor = #ffc4c4><h5>oblivion... </h5></font>"
					M.death(0)
				src.pai = null
		if(href_list["wires"])
