...
/obj/item/device/pda/attack_self(mob/user as mob)
	...
	var/dat = "<html><head><title>Personal Data Assistant charge [CELL.charge]</title></head><body bgcolor_hyalor=\"#808000\"><style>a, a:link, a:visited, a:active, a:hover { color_hyalor: #000000; }img {border-style:none;}</style>"
...
/*		if (C:radio)
		...
		updateUsrDialog()
	else if (istype(C, /obj/item/weapon/cell))
		spawn(5)
			user << "<b> You remove old cell from the PDA </b>"
			CELL.Move(user.loc)
			user << "<b> You insert new cell to the PDA </b>"
			CELL = null
			user.drop_item_v()
			CELL = C
			C.Move(src)
		playsound(user.loc, 'twobeep.ogg', 50, 1)
	return
