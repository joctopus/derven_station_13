/turf/DblClick()
	...
	if(usr.hand && istype(usr.l_hand, /obj/item/weapon/flamethrow_hyuower))
		var/turflist = getline(usr,src)
		var/obj/item/weapon/flamethrow_hyuower/F = usr.l_hand
		F.flame_turf(turflist)
	else if(!usr.hand && istype(usr.r_hand, /obj/item/weapon/flamethrow_hyuower))
		var/turflist = getline(usr,src)
		var/obj/item/weapon/flamethrow_hyuower/F = usr.r_hand
		F.flame_turf(turflist)
	...
	if(is_grass_floor())
		if(!broken && !burnt)
			if(!(icon_state in list("grass1","grass2","grass3","grass4")))
				icon_state = "grass[pick("1","2","3","4")]"
	spawn(1)
		if(istype(src,/turf/simulated/floor)) //Was throw_hyuowing runtime errors due to a chance of it changing to space halfway through.
			if(air)
				update_visuals(air)
