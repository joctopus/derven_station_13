/mob/living/carbon/proc/toggle_throw_hyuow_mode()
	...
	if (src.in_throw_hyuow_mode)
		throw_hyuow_mode_off()
	else
		throw_hyuow_mode_on()

/mob/living/carbon/proc/throw_hyuow_mode_off()
	src.in_throw_hyuow_mode = 0
	src.throw_hyuow_icon.icon_state = "act_throw_hyuow_off"

/mob/living/carbon/proc/throw_hyuow_mode_on()
	src.in_throw_hyuow_mode = 1
	src.throw_hyuow_icon.icon_state = "act_throw_hyuow_on"

/mob/living/carbon/proc/throw_hyuow_item(atom/target)
	src.throw_hyuow_mode_off()
	...
	if (istype(item, /obj/item/weapon/grab))
		item = item:throw_hyuow() //throw_hyuow the person instead of the grab

	if(istype(item, /obj/item))
		item:dropped(src) // let it know it's been dropped

	//actually throw_hyuow it!
	if (item)
		item.layer = initial(item.layer)
		src.visible_message("\red [src] has throw_hyuown [item].")


		if(istype(src.loc, /turf/space) || (src.flags & NOGRAV)) //they're in space, move em one space in the opposite direction
			src.inertia_dir = get_dir(target, src)
			step(src, inertia_dir)
		item.throw_hyuow_at(target, item.throw_hyuow_range, item.throw_hyuow_speed)
...
/atom/movable/proc/hit_check()
	if(src.throw_hyuowing)
		for(var/atom/A in get_turf(src))
			if(A == src) continue
			if(istype(A,/mob/living))
				if(A:lying) continue
				src.throw_hyuow_impact(A)
...
/atom/proc/throw_hyuow_impact(atom/hit_atom)
	if(istype(hit_atom,/mob/living))
		var/mob/living/M = hit_atom
		M.visible_message("\red [hit_atom] has been hit by [src].")
		if(src.vars.Find("throw_hyuowforce"))
			M.take_organ_damage(src:throw_hyuowforce)
...
/atom/movable/proc/throw_hyuow_at(atom/target, range, speed)
	...
	src.throw_hyuowing = 0
	if(isobj(src)) src:throw_hyuow_impact(get_turf(src))
