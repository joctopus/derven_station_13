...
/obj/item/clothing/suit/space/space_ninja/proc/display_spideros()
	...
	var/dat = "<html><head><title>SpiderOS</title></head><body bgcolor_hyalor=\"#3D5B43\" text=\"#DB2929\"><style>a, a:link, a:visited, a:active, a:hover { color_hyalor: #DB2929; }img {border-style:none;}</style>"
	...
	switch(spideros)
		...
		if(4)
			dat += {"
					...
					<li>*<b>Energy Blade</b> (<i>500E</i>) is a highly effective weapon. It is summoned directly to the ninja's hand and can also function as an EMAG for certain objects (doors/lockers/etc). You may also use it to cut through walls and disabled doors. Experiment! The blade will crit humans in two hits. This item cannot be placed in containers and when dropped or throw_hyuown disappears. Having an energy blade drains more power from the battery each tick.</li>
...
/obj/effect/energy_net
	...
	process(var/mob/living/carbon/M as mob)
		...
		if(!isnull(src))//As long as both net and person exist.
			...
			M.loc = pick(holdingfacility)//throw_hyuow mob in to the holding facility.
