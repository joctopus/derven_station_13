	...
	Entered(var/obj/mecha/mecha)
		...
		if(istype(mecha))
			...
			if(recharge_console && recharge_port)
				...
				mecha.occupant_message("<font color_hyalor='red'>Control console not found. Terminating.</font>")
			else if(!recharge_port)
				mecha.occupant_message("<font color_hyalor='red'>Power port not found. Terminating.</font>")
		return
...
/obj/machinery/mech_bay_recharge_port
	...
	proc/start_charge(var/obj/mecha/recharging_mecha)
		if(stat&(NOPOWER|BROKEN))
			recharging_mecha.occupant_message("<font color_hyalor='red'>Power port not responding. Terminating.</font>")
			return 0
		else
	...
	process(var/obj/machinery/mech_bay_recharge_port/port, var/obj/mecha/mecha)
		...
		if(mecha && mecha in port.recharge_floor)
			...
			if(delta>0)
				...
				mecha.occupant_message("<font color_hyalor='blue'><b>Fully charged.</b></font>")
				port.stop_charge()
		else
			port.stop_charge()
		return
...
/obj/machinery/computer/mech_bay_power_console
	...
	proc/mecha_in(var/obj/mecha/mecha)
		if(stat&(NOPOWER|BROKEN))
			mecha.occupant_message("<font color_hyalor='red'>Control console not responding. Terminating...</font>")
			return
	...
	attack_hand(mob/user as mob)
		...
		if(!recharge_floor)
			output += "<font color_hyalor='red'>Mech Bay Recharge Station not initialized.</font><br>"
		else
			...
			output += "</div>"
		if(!recharge_port)
			output += "<font color_hyalor='red'>Mech Bay Power Port not initialized.</font><br>"
		else
