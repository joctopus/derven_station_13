			...
			if(1)
				...
				if(!locked)
					...
					if(locked in view(chassis))
						locked.throw_hyuow_at(target, 14, 1.5)
						locked = null
						send_byjax(chassis.occupant,"exosuit.browser","\ref[src]",src.get_equip_info())
						set_ready_state(0)
						chassis.use_power(energy_drain)
...
/obj/item/mecha_parts/mecha_equipment/anticcw_armor_booster //what is that noise? A BAWWW from TK mutants.
	...
	get_equip_info()
		if(!chassis) return
		return "<span style=\"color_hyalor:[equip_ready?"#0f0":"#f00"];\">*</span>&nbsp;[src.name]"
...
/obj/item/mecha_parts/mecha_equipment/antiproj_armor_booster
	...
	proc/dynhitby(atom/movable/A)
		...
		if(prob(chassis.deflect_chance*deflect_coeff) || istype(A, /mob/living) || istype(A, /obj/item/mecha_tracking))
			...
			var/obj/O = A
			if(O.throw_hyuowforce)
				chassis.take_damage(round(O.throw_hyuowforce*damage_coeff))
				chassis.check_for_internal_damage(list(MECHA_INT_TEMP_CONTROL,MECHA_INT_TANK_BREACH,MECHA_INT_CONTROL_LOST))
		set_ready_state(0)
		chassis.use_power(energy_drain)
		do_after_cooldown()
...
/obj/item/mecha_parts/mecha_equipment/repair_droid
	...
	get_equip_info()
		if(!chassis) return
		return "<span style=\"color_hyalor:[equip_ready?"#0f0":"#f00"];\">*</span>&nbsp;[src.name] - <a href='?src=\ref[src];toggle_repairs=1'>[pr_repair_droid.active()?"Dea":"A"]ctivate</a>"
...
/obj/item/mecha_parts/mecha_equipment/tesla_energy_relay
	...
	get_equip_info()
		if(!chassis) return
		return "<span style=\"color_hyalor:[equip_ready?"#0f0":"#f00"];\">*</span>&nbsp;[src.name] - <a href='?src=\ref[src];toggle_relay=1'>[pr_energy_relay.active()?"Dea":"A"]ctivate</a>"
