...
/obj/item/mecha_parts/mecha_equipment/weapon/honker
	...
	action(target)
		...
		chassis.occupant_message("<font color_hyalor='red' size='5'>HONK</font>")
		for(var/mob/living/carbon/M in ohearers(6, chassis))
			if(istype(M, /mob/living/carbon/human))
				var/mob/living/carbon/human/H = M
				if(istype(H.ears, /obj/item/clothing/ears/earmuffs))
					continue
			M << "<font color_hyalor='red' size='7'>HONK</font>"
			M.sleeping = 0
			M.stuttering += 20
			M.ear_deaf += 30
			M.Weaken(3)
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/missile_rack
	...
	action(target)
		...
		M.throw_hyuow_at(target, missile_range, missile_speed)
		projectiles--
		chassis.log_message("Fired from [src.name], targeting [target].")
		do_after_cooldown()
		return
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/missile_rack/flashbang
	...
	action(target)
		...
		playsound(chassis, 'bang.ogg', 50, 1)
		F.throw_hyuow_at(target, missile_range, missile_speed)
		projectiles--
		chassis.log_message("Fired from [src.name], targeting [target].")
		spawn(det_time)
			F.prime()
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/missile_rack/banana_mortar
	...
	action(target)
		...
		B.throw_hyuow_at(target, missile_range, missile_speed)
		projectiles--
