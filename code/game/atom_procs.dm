...
// TODO DOOHL: Intergrate params to new proc. Saved for another time because var/valid_place is a fucking brainfuck
	...
	if(ishuman(usr))
		...
		if(human.in_throw_hyuow_mode)
			return human.throw_hyuow_item(src)
		...
		if(monkey.in_throw_hyuow_mode)
			return monkey.throw_hyuow_item(src)
...
//		world << "atom.DblClick() on [src] by [usr] : src.type is [src.type]"
	...
	// ------- throw_hyuow -------
	if(usr.in_throw_hyuow_mode)
		return usr:throw_hyuow_item(src)
