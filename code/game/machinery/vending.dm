	...
	if(rebuild) // rebuild the buttons and shit
		for(var/i = 1, i <= PRODUCTS_PER_PAGE, i++) // "refresh" everything to default state
			...
			if(i % 2 == 0) // even number (remainder of i / 2 == 0)
				winset(user, "vendingwindow_n.product[i]", "background-color_hyalor=#78A6C7") // Specific even color_hyalor
			else
				winset(user, "vendingwindow_n.product[i]", "background-color_hyalor=#8BA5B4") // Odd color_hyalor
	...
	for (var/i = 1, i <= products.len, i++) // loop through all products
		...
		if(product)
			...
			if(product.amount <= 0)
				...
				winset(user, "vendingwindow_n.product[i]", "background-color_hyalor=#526F7C") // Darken the button
	...
	for (i = 0, i < 6, i++)
		if (products.len >= base + i)
			...
			if (product.amount > 0)
				...
				winset(user, "vendingslot[i+1].stock", "text-color_hyalor=\"#000000\"")
				winshow(user, "vendingslot[i+1].buy", 1)
			else
				winset(user, "vendingslot[i+1].stock", "text=\"OUT OF STOCK\"")
				winset(user, "vendingslot[i+1].stock", "text-color_hyalor=\"#FF0000\"")
...
/obj/machinery/vending/attack_hand(mob/user as mob)
	...
	if(panel_open)
		...
		for(var/wiredesc in vendwires)
			var/is_uncut = src.wires & APCWirecolor_hyalorToFlag[vendwires[wiredesc]]
			dat += "[wiredesc] wire: "
			if(!is_uncut)
	...
	if ((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))))
		...
		if ((href_list["vend"]) && (src.vend_ready))
			...
			if (!( istype(usr.equipped(), /obj/item/weapon/wirecutters) ))
				...
				return
			if (src.isWirecolor_hyalorCut(twire))
				src.mend(twire)
			else
				src.cut(twire)
				updateWindow(usr)
...
/obj/machinery/vending/proc/throw_hyuow_item()
	var/obj/throw_hyuow_item = null
	...
	for(var/datum/data/vending_product/R in src.product_records)
		...
		R.amount--
		throw_hyuow_item = new dump_path(src.loc)
		break
	if (!throw_hyuow_item)
		return 0
	spawn(0)
		throw_hyuow_item.throw_hyuow_at(target, 16, 3)
	src.visible_message("\red <b>[src] launches [throw_hyuow_item.name] at [target.name]!</b>")
	return 1

/obj/machinery/vending/proc/isWirecolor_hyalorCut(var/wirecolor_hyalor)
	var/wireFlag = APCWirecolor_hyalorToFlag[wirecolor_hyalor]
...
/obj/machinery/vending/proc/cut(var/wirecolor_hyalor)
	var/wireFlag = APCWirecolor_hyalorToFlag[wirecolor_hyalor]
	var/wireIndex = APCWirecolor_hyalorToIndex[wirecolor_hyalor]
	src.wires &= ~wireFlag
	switch(wireIndex)
		if(WIRE_EXTEND)
...
/obj/machinery/vending/proc/mend(var/wirecolor_hyalor)
	var/wireFlag = APCWirecolor_hyalorToFlag[wirecolor_hyalor]
	var/wireIndex = APCWirecolor_hyalorToIndex[wirecolor_hyalor] //not used in this function
	src.wires |= wireFlag
	switch(wireIndex)
//		if(WIRE_SCANID)
		if(WIRE_SHOCK)
...
/obj/machinery/vending/proc/pulse(var/wirecolor_hyalor)
	var/wireIndex = APCWirecolor_hyalorToIndex[wirecolor_hyalor]
	switch(wireIndex)
		if(WIRE_EXTEND)
