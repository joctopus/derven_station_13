...
/proc/RandomAAlarmWires()
	...
	AAlarmIndexToWirecolor_hyalor = list(0, 0, 0, 0, 0)
	AAlarmWirecolor_hyalorToIndex = list(0, 0, 0, 0, 0)
	var/flagIndex = 1
	for (var/flag=1, flag<32, flag+=flag)
		var/valid = 0
		while (!valid)
			var/color_hyalorIndex = rand(1, 5)
			if (AAlarmwires[color_hyalorIndex]==0)
				valid = 1
				AAlarmwires[color_hyalorIndex] = flag
				AAlarmIndexToFlag[flagIndex] = flag
				AAlarmIndexToWirecolor_hyalor[flagIndex] = color_hyalorIndex
				AAlarmWirecolor_hyalorToIndex[color_hyalorIndex] = flagIndex
		flagIndex+=1
	return AAlarmwires
...
/obj/machinery/alarm/attack_hand(mob/user)
	...
	if(wiresexposed && (!istype(user, /mob/living/silicon)))
		...
		for(var/wiredesc in AAlarmwires)
			var/is_uncut = src.AAlarmwires & AAlarmWirecolor_hyalorToFlag[AAlarmwires[wiredesc]]
			t1 += "[wiredesc] wire: "
			if(!is_uncut)
...
/obj/machinery/alarm/proc/isWirecolor_hyalorCut(var/wirecolor_hyalor)
	var/wireFlag = AAlarmWirecolor_hyalorToFlag[wirecolor_hyalor]
...
/obj/machinery/alarm/proc/cut(var/wirecolor_hyalor)
	var/wireFlag = AAlarmWirecolor_hyalorToFlag[wirecolor_hyalor]
	var/wireIndex = AAlarmWirecolor_hyalorToIndex[wirecolor_hyalor]
...
/obj/machinery/alarm/proc/mend(var/wirecolor_hyalor)
	var/wireFlag = AAlarmWirecolor_hyalorToFlag[wirecolor_hyalor]
	var/wireIndex = AAlarmWirecolor_hyalorToIndex[wirecolor_hyalor] //not used in this function
	AAlarmwires |= wireFlag
	switch(wireIndex)
		if(AALARM_WIRE_IDSCAN)
...
/obj/machinery/alarm/proc/pulse(var/wirecolor_hyalor)
	//var/wireFlag = AAlarmWirecolor_hyalorToFlag[wirecolor_hyalor] //not used in this function
	var/wireIndex = AAlarmWirecolor_hyalorToIndex[wirecolor_hyalor]
	switch(wireIndex)
...
/obj/machinery/alarm/proc/return_status()
	...
	output += {"
<style>
.dl0 { color_hyalor: green; }
.dl1 { color_hyalor: orange; }
.dl2 { color_hyalor: red; font-weght: bold;}
</style>
"}
