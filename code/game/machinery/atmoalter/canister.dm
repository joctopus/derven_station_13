...
/obj/machinery/portable_atmospherics/canister/update_icon()
	...
	if (src.destroyed)
		src.icon_state = text("[]-1", src.color_hyalor)

	else
		icon_state = "[color_hyalor]"
		if(holding)
	...
	if (((get_dist(src, usr) <= 1) && istype(src.loc, /turf)))
		...
		if(href_list["toggle"])
			if (valve_open)
				if (holding)
					...
					release_log += "Valve was <b>closed</b> by [usr], stopping the transfer into the <font color_hyalor='red'><b>air</b></font><br>"
			else
				if (holding)
					release_log += "Valve was <b>opened</b> by [usr], starting the transfer into the [holding]<br>"
				else
					release_log += "Valve was <b>opened</b> by [usr], starting the transfer into the <font color_hyalor='red'><b>air</b></font><br>"
		...
		if (href_list["relabel"])
			if (!is_labeled)
				...
				var/label = input("Choose canister label", "Gas canister") as null|anything in color_hyalors
				if (label)
					src.color_hyalor = color_hyalors[label]
					src.icon_state = color_hyalors[label]
