		...
		if (authenticated && modify)
			var/carddesc = {"<script type="text/javascript">
								function markRed(){
									...
									nameField.style.backgroundcolor_hyalor = "#FFDDDD";
								}
								function markGreen(){
									var nameField = document.getElementById('namefield');
									nameField.style.backgroundcolor_hyalor = "#DDFFDD";
								}
								function showAll(){
			...
			carddesc += "<b>Registered:</b> <input type='text' id='namefield' name='reg' value='[target_owner]' style='width:250px; background-color_hyalor:white;' onchange='markRed()'>"
			...
			if(istype(src,/obj/machinery/computer/card/centcom))
				...
				for(var/A in get_all_centcom_access())
					if(A in modify.access)
						accesses += "<a href='?src=\ref[src];choice=access;access_target=[A];allowed=0'><font color_hyalor=\"red\">[dd_replacetext(get_centcom_access_desc(A), " ", "&nbsp")]</font></a> "
					else
				...
				for(var/i = 1; i <= 7; i++)
					...
					for(var/A in get_region_accesses(i))
						if(A in modify.access)
							accesses += "<a href='?src=\ref[src];choice=access;access_target=[A];allowed=0'><font color_hyalor=\"red\">[dd_replacetext(get_access_desc(A), " ", "&nbsp")]</font></a> "
						else
...
/obj/machinery/mass_driver/proc/drive(amount)
	...
	for(var/atom/movable/O in loc)
		if(!O.anchored||istype(O, /obj/mecha))//Mechs need their launch platforms.
			...
			use_power(500)
			spawn( 0 )
				O.throw_hyuow_at(target, drive_range * power, power)
